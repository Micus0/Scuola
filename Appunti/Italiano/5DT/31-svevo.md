# Italiano
## Italo Svevo

- non si chiama realmente cosi
- svevo scrive male
  - alfabetizzazione influenzata da dove vive e formazione

## Vita

- vero nome: **Aron Hector Schmitz**
  - Italo Svevo
    - **italia-sveva**(germania)
- **nasce a Trieste** (1861)
  - quando nasce era **sotto impero austro-ungarico**
- famiglia ebraica di commercianti
- studia in **germania** e studia tedesco
  - vede talento scrivere
- impiego monotono
- **primo romanzo**
  - **Una vita**
  - poco successo
  - occasione in cui si chiama Italo Svevo
- vita vari lutti
- punti in comune tra trame e vita
  - es sposare donna con dote tema che si ripete
- **secondo romanzo**
  - **Senilità**
- **ripetizioni di inglese da Joyce**
  - viveva a Trieste
  - **influenzato** a livello letterario
  - incontro che produce cultura
- esperienza ravvicinata psicoanalisi
- **terzo romanzo**
  - tutti a spese dell'autore
  - **La coscienza di Zeno**
  - Montale recensisce positivamente
- 1928 inizia quarto romanzo
  - ma **muore in un incidente d'auto**
    - ci avviciniamo ad elementi contemporaneità
- caratteristiche imp
  - formazione non quella elevata degli altri intellettuali
    - formazione semplice
  - bisogno di scrivere per dare voce ad una parte di sé
    - imp incontro Joyce e psicanalisi
  - influenze culturali imp
    - italia-austria-slava
    - e provenienza famiglia ebraica
    - --> crogiolo di culture/lingue
  - natura commerciale Trieste
    - professioni non coincidono con la figura di scrittore/intellettuale
    - studi orientamento commerciale
      - ma determinanti

Visione mondo
- dominatori e inetti
  - non proprio d'annunzio
    - non c'è questa divisione in categorie
      - superuomo in un individuo che si eleva e basta
  - no verga
  - --> lotta per la sopravvivenza da darwin
    - selezione naturale
      - personaggi si comportano in una certa maniera dettata da leggi naturali che non dipendono esclusivamente dalla volontà dei personaggi, ma dai rapporti sociali
      - quindi non solo prodotto di natura, ma un prodotto storico, in particolare puntando una luce sulla borghesia
      - importante apporto di freud che è coevo (contemporanei) per farlo indagari suoi conflitti interiori (non solo di classe), ma svevo non ci da lo studio della psicologia individuale in astratto, ma ci da lo studio/analisi di una psicologia che metta in luce i conflitti tipicamente borghesi
      - spiegazione
        - comportamento prodotto da un contesto sociale che determina questo --> + storico che naturale
        - presa in oggetto quella a cui appartiene: classe borghese
        - classi sociale bassi lotta per la vita + forte (come anche verga)
        - ma lui sulla borghesia in un periodo storico
          - es per essere banalmente accettato: sposarsi
- si capisce bene dalle trame dei romanzi

Influenze culturali
- Nietzche
- darwinismo
- marxismo
  - conflitto tra le classi imprime fondamentalmente segni di anatomia psicologica
  - no psicologia astratta ma è il ritratto di tutti quei conflitti/ambiguità tipiche della borghesia di quel periodo
    - non profilo in valore assoluto ma solo l'inetto borghese specifico
  - approfondimento per chi vuole qualcosa di più
    - la tribù: metafora pos socialiste di turati...
- psicanalisi
  - no strumento di guarigione/terapia
  - ma strumento conoscitivo --> potente strumento narrativo
    - "grande uomo quel nostro Freud, ma più per i romanzieri che per gli ammalati"
  - imp: cercare tratti malattia in svevo (nel libro da legg)
- Joyce*
  - in inglese fatto meglio



## Trame romanzi

Una vita
- protagonista Alfonso Nitti
- si trasferisce a Trieste
- [..]
- simile alla sua biografia
- impregnato dalla borghesia, 

Senilità
- protagonista Emilio Brentani
- impiegato pubblica mediocre romanzo a proprie spese
- atratto da Angiolina dai facili costumi
  - primon intento è di educarla
    - si fa aiutare da Stefano Balli
- ma lei prova a sedurre lui
- fallimento finale
  - non suicidio
  - rimane a contemplare il suo ricordo
- sempre simile alla sua biografia
  - tutti e due degli inetti

La coscienza di Zeno
- narrazione non più fabula intreccio lineare
- ma blocchi tematici
- solo alla fine raccontare ordine cronologico
- [..]
- sembra aver scritto 3 volte lo stesso romanzo o sua vita
  - ma evoluzione vita e cambiano cose
- cosa parla coscienza o incoscienza?
- metodo ricordare romanzi
  - cognomi ABC

Elementi
- simili
- distanti

## Una vita

- inaugura personaggio inetto
- debolezza/insicurezza psicologica
- --> incapace e impreparato alla vita
- è originale?
  - inetto ennesima reazione come fanciullino e superuomo
- debolezza negli aspetti sociali
- Alfonso Nitti
  - piccolo borghese
  - dedito e amante della cultura
  - dissesto nella collocazione sociale
    - escluso --> inadeguatezza/inettitudine
  - != modello di forza a cui si guarda con immirazione
    - guarda contemplivamente il suo protagonista
- romanzo psicologico
  - come anche il piacere
  - ma con sequenze riflessive
    - qui tutto innervato cosi
- critica lo stronca/insuccesso
  - pubblico del tempo non ha le coordinate per comprenderlo
    - Freud poi al tempo non è così conosciuto in Italia
- focalizzazione interna

## Senilità

- più insuccesso del primo
- Emilio Brentani
  - anche lui borghese intellettuale
  - spinto al godimento piacere
    - incarna nella figura di Angiolina
      - ma relazione in cui emerge ancora di più la sua inettitudine
- anche qui presente il dominatore
  - caratteristiche del superuomo
- atteggiamento fortemente critico dell'autore verso i personaggi
  - interventi espliciti narratore
    - voce fuori campo
    - per smascherare autoinganni
      - ci si giustifica per assolversi
        - meccanismo psicologico
- raccontare questi meccanismi potere conoscitivo/narrativo

## La coscienza di Zeno

- coordinate per leggere le parti del libro
  - esce nel 1923
    - WWI
    - teoria relatività
    - impatto sul sentire comune dell'uomo
    - in qualcosa si può trovare riflessione guerra (alla fine la faremo)
  - tempo misto
    - nuclei tematici
    - tempo della coscienza
      - ep lunghi --> corti o vice versa
  - attenzione attendibilità/inattendibilità narratore
    - la sua coscienza lo vede così
    - parte già dall'inizio nella prefazione
    - cercare autoinganni/incoscienza
  - chi considera sano e quante volte si considera malato
  - attenzione forma che sembra che non scorra
    - costrutti che sembra scriva male
      - es concordanze che noi non usiamo
        - io volevo di andare
